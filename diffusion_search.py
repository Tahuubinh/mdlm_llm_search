...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    self.num_diffusion_steps = self.config.sampling.steps
    self.best_clean_samples = None
    self.posterior_sampling_method = getattr(config.sampling, 'posterior_sampling_method')
    num_posterior_samples = getattr(config.sampling, 'num_posterior_samples')
    num_mcts_samples = getattr(config.sampling, 'num_mcts_samples')
    property_type = getattr(config.sampling, 'property_type')
    lower_bound = getattr(config.sampling, 'lower_bound')
    upper_bound = getattr(config.sampling, 'upper_bound')
    none_distances = getattr(config.sampling, 'none_distances')

    num_local_searches = getattr(config.sampling, 'num_local_searches')
    top_k_categorical = getattr(config.sampling, 'top_k_categorical')
    nucleus = getattr(config.sampling, 'nucleus')
    self.x_theta_modifier_method = getattr(config.sampling, 'x_theta_modifier_method')
    ...
    sampler = SAMPLING_METHOD_CLASSES[self.posterior_sampling_method](
        ...
        none_distances=none_distances,
        num_local_searches=num_local_searches,
    ...
    self._sample_from_posterior = sampler.get_sampling_method()

    x_theta_modifier = X_THETA_MODIFIER_CLASSES[self.x_theta_modifier_method](
        num_x_theta_samples=num_x_theta_samples,
        property_type=property_type,
        lower_bound=lower_bound,
        upper_bound=upper_bound,
        num_local_searches=x_theta_num_local_searches,
        tokenizer=self.tokenizer,
        top_k_categorical=top_k_categorical,
        num_diffusion_steps=self.num_diffusion_steps,
        data=self.config.data.train,
  ...
  @torch.no_grad()
  def sample(self, num_steps=None, eps=1e-5, prefixes=None):
    """Public method to generate samples with optional prefix conditioning.
    
    Args:
        num_steps: Number of diffusion steps (default: use config)
        eps: Minimum time value (default: 1e-5)
        prefixes: Optional list of (prefix_text, prefix_token_ids) tuples for conditioning
        
    Returns:
        Generated token sequences as torch.Tensor
    """
    ...
    # Handle prefix conditioning for toxicity generation
    if prefixes is not None:
      batch_size = len(prefixes)
      # Initialize with mask tokens
      x = self._sample_prior(batch_size, self.config.model.length).to(self.device)
      
      # Fill in prefix tokens (they should not be masked)
      for idx, (prefix_text, prefix_token_ids) in enumerate(prefixes):
        prefix_len = min(len(prefix_token_ids), self.config.model.length)
        x[idx, :prefix_len] = torch.tensor(prefix_token_ids[:prefix_len], 
                                           dtype=torch.long, device=self.device)
    else:
      # Standard initialization with all mask tokens
      x = self._sample_prior(batch_size, self.config.model.length).to(self.device)
    ...
    for i in tqdm(range(num_steps), desc="Sampling steps", leave=False):
      ...
      # Compute log probabilities for x_theta modification and posterior sampling
      ...
      if self.sampler == 'ddpm':
        ...
        # Compute q_xs for posterior sampling
        move_chance_t = 1 - torch.exp(-sigma_t)
        sigma_s, _ = self.noise(t - dt)
        if sigma_s.ndim > 1:
          sigma_s = sigma_s.squeeze(-1)
        ...
        # Apply posterior sampling if configured
        # Signature: _sample_from_posterior(q_xs, xt, step)
        x_next = self._sample_from_posterior(q_xs, x, i)
        
        # Apply copy flag for absorbing state
        copy_flag = (x != self.mask_index).to(torch.bool)
        x_next = torch.where(copy_flag, best_clean_samples if best_clean_samples is not None else x, x_next)
      ...
      # Maintain prefix conditioning throughout sampling
      if prefixes is not None:
        for idx, (prefix_text, prefix_token_ids) in enumerate(prefixes):
          prefix_len = min(len(prefix_token_ids), self.config.model.length)
          # Keep prefix tokens fixed
          x[idx, :prefix_len] = torch.tensor(prefix_token_ids[:prefix_len],
                                            dtype=torch.long, device=self.device)
    ...
    if self.config.sampling.noise_removal:
      ...
      # Maintain prefix after final denoising
      if prefixes is not None:
        for idx, (prefix_text, prefix_token_ids) in enumerate(prefixes):
          prefix_len = min(len(prefix_token_ids), self.config.model.length)
          x[idx, :prefix_len] = torch.tensor(prefix_token_ids[:prefix_len],
